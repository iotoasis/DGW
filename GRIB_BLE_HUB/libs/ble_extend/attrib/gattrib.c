								...
								gpointer data)
{
	...
	if (iostat != G_IO_STATUS_NORMAL) {
		if (gerr) {
			...
			error("can_write_data: %s", gerr->message);
			printf("# can_write_data: %s", gerr->message);
			g_error_free(gerr);
		}
...
GAttrib *g_attrib_new(GIOChannel *io)
{
	...
	if (gerr) {
		g_printerr("# BLE-EX: [FILE: %s] [FUNC: %s]\n", __FILE_NAME__, __FUNC_NAME__);
		error("g_attrib_new: %s", gerr->message);
		printf("# g_attrib_new: %s", gerr->message);
		g_error_free(gerr);
		return NULL;
	}
