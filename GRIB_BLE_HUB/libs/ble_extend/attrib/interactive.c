...
static GError* gLibError;
static Grib_BleErrorCode gErrorCode;
...
static void connect_cb(GIOChannel *io, GError *err, gpointer user_data)
{
	if (err) {
		...
		printf("# INTERACTIVE CONNECT CB: %s\n", err->message);
		set_state(STATE_DISCONNECTED);

		Grib_BleCommLoopQuit(event_loop, err, BLE_ERROR_CODE_INTERACTIVE);
		return;
	}
...
static void cmd_exit(int argcp, char **argvp)
{
	...
	for(i=0; i<argcp; i++)
	{
		printf("# INTERACTIVE CMD EXIT[%d/%d]: %s\n", i, argcp, argvp[i]);
	}

	rl_callback_handler_remove();

	Grib_BleCommLoopQuit(event_loop, gLibError, gErrorCode);
...
static void cmd_sec_level(int argcp, char **argvp)
{
	...
	if (gerr) {
		gLibError = gerr;
		gErrorCode = BLE_ERROR_CODE_INTERACTIVE;
		g_printerr("# BLE-EX: [FILE: %s] [FUNC: %s]\n", __FILE_NAME__, __FUNC_NAME__);
		printf("# cmd_sec_level: %s\n", gerr->message);
		g_error_free(gerr);
	}
}
		...
		const gchar *dst_type, int psm)
{
	...
	gErrorCode = BLE_ERROR_CODE_NONE;
	printf("# BLE-EX: [FILE: %s] [FUNC: %s]\n", __FILE_NAME__, __FUNC_NAME__);
