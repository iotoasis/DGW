...
int myControl(GribFuncParam* pParam);
//# ########## ########## ########## ########## ########## ########## ########## ##########
...
int setPower(int iPower)
{
    int iError = RES_ERROR;
    int iValue = 0;

    digitalWrite(SENSOR_PIN, iPower);
    setMyLed(iPower);
...
int myControl(GribFuncParam* pParam)
{
    ...
    iValue = atoi(&strchr(pParam->recvMsg, '=')[1]);
    if (gDebug) {
        Serial.print("# CONTROL DATA: ");
        Serial.println(iValue);
    }

    if ( (SENSOR_VALUE_MIN <= iValue) && (iValue <= SENSOR_VALUE_MAX) )
    {
        setPower(iValue);
        snprintf(pParam->sendMsg, MAX_SIZE_BT_BUFF, "%s", RES2STRING(RES_OK));
    }
    else
    {
        iError = RES_ERROR;
...
void loop()
{
    ...
    if (btSerial.available())
    {
        ...
        if (gDebug) {
            Serial.print("# BT SEND LEN: ");
            Serial.println(strlen(pParam.sendMsg));
        }
    ...
    if (Serial.available())
    {
        ...
        Serial.print(F("# SERIAL INPUT  MSG: "));
        Serial.println(gRecvBuff);
